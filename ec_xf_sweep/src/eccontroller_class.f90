!>
!> Originally this module both managed hitstats and served as a controller
!> for the HIT state; in this version it is only a controller, and the hitstats
!> module has been broken out of this module into simulation.f90; results from
!> hitstats and xflowstats are passed into the methods in this module.
!>
!>
module eccontroller_class
  ...
  public :: eccontroller, eccontroller_mesh, ETAODX, RHOF, FORCE_TIMESCALE
  ...
  end type eccontroller_mesh
  interface eccontroller_mesh; procedure ecmesh_from_args; end interface;
...
contains
  ...
  subroutine get_next_params(ec, params, done)
    ...
    implicit none
    class(eccontroller_mesh), intent(inout) :: ec
  ...
  subroutine get_interval(ec, interval)
    ...
    real(WP) :: tinf_interval, xf_interval, urms
    character(len=80) :: s

    urms = sqrt(2.0_WP / 3 * ec%param_target(3))

    tinf_interval = ec%interval_tinfs * FORCE_TIMESCALE
    xf_interval = ec%interval_flowthroughs * ec%xflen / (urms * ec%xfrmsmult)
    ...
    if (ec%sim_pg%amroot) then
      if (interval .eq. tinf_interval) then
        write(s,'(A,E18.8)') "[EC] interval is tinf limited - ", interval
      else
        write(s,'(A,E18.8)') "[EC] interval is xf limited - ", interval
      end if
      call log(s)
    end if
  ...
  subroutine update_write(ec, Re_lambda, Stk, phiinf, Wovk, urms, eta, time, step)
    implicit none
    class(eccontroller), intent(inout) :: ec
  ...
  end subroutine update_write

end module eccontroller_class
