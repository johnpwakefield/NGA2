...
contains
  ...
  subroutine setup_ens(d)
    ...
    d%ens_out = ensight(cfg=d%cfg, name=trim(d%desc), skipmkdir=.true.)
  ...
  subroutine simulation_init
    ...
    create_directories: block
      ...
      if (amroot) then
        ...
        call execute_command_line("mkdir -p ./ensight/cube/velocity")
        call execute_command_line("mkdir -p ./ensight/cube/pressure")
        call execute_command_line("mkdir -p ./ensight/cube/particles")
        call execute_command_line("mkdir -p ./ensight/xflw/velocity")
        call execute_command_line("mkdir -p ./ensight/xflw/pressure")
        call execute_command_line("mkdir -p ./ensight/xflw/particles")
      end if
  ...
  subroutine simulation_run
    ...
    do while (.not. (time%done() .or. ec_done))
      ...
      do while (.not. ec_evt%occurs())
        ...
        call xflw%fs%get_strainrate(SR=xflw%SR)
