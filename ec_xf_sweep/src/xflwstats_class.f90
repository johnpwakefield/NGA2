...
module xflwstats_class
  ...
  type :: xdepstats
    integer :: step
    integer, dimension(25) :: units
    ! these arrays are only imin to imax and don't include overlap
    integer, dimension(:), allocatable :: num
    real(WP), dimension(:), allocatable :: nden, ndensq, p, pke, fke, eps
...
contains
  ...
  subroutine xdepstats_init(this, g, mkdir)
    ...
    allocate(this%num(g%imin:g%imax),                                         &
      ...
      this%fvel(3,g%imin:g%imax), this%pke(g%imin:g%imax),                    &
      this%fke(g%imin:g%imax), this%eps(g%imin:g%imax))
    allocate(this%tmpmesh(g%imin_:g%imax_,g%jmin_:g%jmax_,g%kmin_:g%kmax_))
    ...
    if (g%rank .eq. 0) then
      ...
      open(newunit=this%units(24), action='WRITE', form='FORMATTED',          &
        file='xflw_stat/fke')
      open(newunit=this%units(25), action='WRITE', form='FORMATTED',          &
        file='xflw_stat/eps')
  ...
  subroutine xdepstats_write(this, g)
    ...
    if (g%rank .eq. 0) then
      ...
      write(this%units(25),'(I8,*(E20.8E3))') n, this%eps(     g%imin:g%imax)
  ...
  subroutine xflwstats_init(this, sim_cfg, filterfile, FFTN, vf, rhof, p, visc, U, V, W, SR, ps)
    ...
    real(WP), dimension(1:,sim_cfg%imino_:,sim_cfg%jmino_:,sim_cfg%kmino_:),  &
      intent(in), target :: SR
    ...
    this%U => U; this%V => V; this%W => W; this%SR => SR;
  ...
  subroutine xflwstats_xdep_stats(this, step)
    ...
    real(WP), dimension(this%ps%cfg%imin:this%ps%cfg%imax) :: press, pke, fke, eps, ndensq
    ...
    call mpi_reduce(ndensq,this%mstats%ndensq,this%ps%cfg%nx,   MPI_REAL_WP,  &
      MPI_SUM, 0, this%ps%cfg%comm)
    ...
    vel(:,:) = 0.0_WP; press(:) = 0.0_WP; fke(:) = 0.0_WP; eps(:) = 0.0_WP;
    do k = this%ps%cfg%kmin_, this%ps%cfg%kmax_
      do j = this%ps%cfg%jmin_, this%ps%cfg%jmax_
        do i = this%ps%cfg%imin_, this%ps%cfg%imax_
          ...
          eps(i) = eps(i) + 2.0_WP*this%visc(i,j,k)*this%ps%cfg%vol(i,j,k)*(   &
            this%SR(1,i,j,k)**2+this%SR(2,i,j,k)**2+this%SR(3,i,j,k)**2+2.0_WP*(  &
            this%SR(4,i,j,k)**2+this%SR(5,i,j,k)**2+this%SR(6,i,j,k)**2)          &
            ) / this%rhof(i,j,k)
        end do
      end do
    end do
    ...
    call mpi_reduce(fke, this%mstats%fke,    this%ps%cfg%nx,   MPI_REAL_WP,   &
      MPI_SUM, 0, this%ps%cfg%comm)
    call mpi_reduce(eps, this%mstats%eps,    this%ps%cfg%nx,   MPI_REAL_WP,   &
      MPI_SUM, 0, this%ps%cfg%comm)
    if (this%ps%cfg%rank .eq. 0) then
      do i = this%ps%cfg%imin, this%ps%cfg%imax
        fke(i) = fke(i) / (this%ps%cfg%yL * this%ps%cfg%zL * this%ps%cfg%dx(i))
        eps(i) = eps(i) / (this%ps%cfg%yL * this%ps%cfg%zL * this%ps%cfg%dx(i))
      end do
