...
contains
   ...
   subroutine simulation_init
      ...
      initialize_timetracker: block
       ...
       initialize_pt: block
         ...
         call param_read('Particle injection rate',pt%inj_rate)
         pt%inj_pos(1)=pt%cfg%x(pt%cfg%imin)
      ...
      create_monitor: block
         ...
         call mfile%add_column(pt%np,'Particle number')
   ...
   subroutine simulation_run
      ...
      do while (.not.time%done())
         ...
         ! Seed and advance tracer particles
         resU=fs%rho
         call pt%inject(dt=time%dtmid,U=fs%U,V=fs%V,W=fs%W,rho=resU,P=fs%P)
         call pt%advance(dt=time%dtmid,U=fs%U,V=fs%V,W=fs%W,rho=resU,P=fs%P)
